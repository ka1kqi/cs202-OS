...
// inode 'ino' would be mapped.  Allocate the file block if it doesn't yet
// exist.
//
//
// --
...
//
// Hint: Use inode_block_walk and alloc_diskblock.
int
...
// Remove a file block from inode ino.  If it's not there, just silently succeed.
// Returns 0 on success, < 0 on error.
static int
inode_free_fileblock(struct inode *ino, uint32_t filebno)
{
	int r;
	uint32_t *ptr;
...
// Remove any file blocks currently allocated for inode "ino" that would
// not be needed for an inode of size "newsize" (where newsize is smaller
// than ino->i_size). Do not change ino->i_size.
//
// For both the old and new sizes, compute the number of file blocks required,
// and then free the file blocks from new_nblocks to old_nblocks. If new_nblocks
...
//
// Hints:
// - Use inode_free_fileblock to free all the data blocks, then use
// free_diskblock to free the meta-data blocks (for example, the indirect block).
